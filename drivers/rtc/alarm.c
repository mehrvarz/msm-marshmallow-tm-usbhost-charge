...
static enum hrtimer_restart alarm_timer_triggered(struct hrtimer *timer)
{
	...
	//	base - alarms, ktime_to_ns(now),usbhost_firm_sleep,suspended); // tmtmtm

	while (base->first) {
		alarm = container_of(base->first, struct alarm, node);
		if (alarm->softexpires.tv64 > now.tv64) {
		  	//pr_info("#:# don't call alarm\n"); // tmtmtm
...
*/
			...
			RB_CLEAR_NODE(&alarm->node);
			if(suspended) {
				//pr_info("#:# call alarm, type %d, func %pF, %lld (s %lld) firm=%d susp=%d\n",
				//	alarm->type, alarm->function,
				//	ktime_to_ns(alarm->expires),
				//	ktime_to_ns(alarm->softexpires),
				//	usbhost_firm_sleep,  // tmtmtm
...
*/
  	...
  	//pr_info("#:# alarm_timer_triggered done loop %d\n",base->first); // tmtmtm
	if (!base->first)
...
static int alarm_resume(struct platform_device *pdev)
{
	...
	// tmtmtm
	suspended = false;
	// delay "suspended = false;" by 10ms, so that immediately following "#:# call alarm" 
